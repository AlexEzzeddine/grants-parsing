...
class GurtSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in links:
            yield scrapy.Request(self.base_url + link,
                                 callback=self.getDetails)

        next_page = self.start_urls[0] +\
            response.css(
                'div.pagination > a.next ::attr(href)').extract_first()
    ...
    def getDetails(self, response):
        ...
        item['title'] = "".join(response.css(
            ".news>h2:first-of-type *::text").extract())
        item['text'] = ''.join(response.css(".newstxt2 *::text").extract())
        item['contacts'] = ' '.join(response.css(
            ".contline+ul li *::text").extract())
        yield item
